...
///
        HwCache<Long, Client> clientCache = new MyCache<>(5, 5);
        HwListener<Long, Client> logListener = (key, client, action) ->
                log.info("cache key: {}, value: {}, action: {}", key, client, action);
        clientCache.addListener(logListener);
        ...
        clientCache.removeListener(logListener);
        log.info("clientCache: {}", clientCache);
    }
}